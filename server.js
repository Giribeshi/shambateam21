...
console.log('Looking for frontend build at:', frontendBuild, '| exists:', fs.existsSync(frontendBuild));
...
if (fs.existsSync(frontendBuild)) {
  ...
  app.get('/', (req, res) => {
    res.send(`
      ...
      .box{text-align:center;max-width:480px;padding:40px;background:#fff;border-radius:16px;box-shadow:0 4px 24px rgba(0,0,0,.08)}
      h2{color:#065f46}p{color:#6b7280}code{background:#f1f5f9;padding:4px 8px;border-radius:6px;font-size:13px}</style></head>
      <body><div class="box">
        <h2>🌱 Shambasmart</h2>
        <p>Server is running but the frontend build is missing.</p>
        <p>Build path checked: <code>${frontendBuild}</code></p>
        <p>The API is functional at <code>/api</code></p>
        <p>Re-deploy with the latest commit to rebuild the frontend.</p>
      </div></body></html>
    `);
  });
}
